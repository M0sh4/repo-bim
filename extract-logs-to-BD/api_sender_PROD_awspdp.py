...
_component = 'api_sender_PROD_awspdp'
...
def calculate_log_group_prefix(manual, date_extract):
    if manual:
        return f"{_component}/{date_extract}"
    else:
        now = datetime.now()
        ...
        return f"{_component}/{cut_off_time.year}/{cut_off_time.month:02}/{cut_off_time.day:02}/{cut_off_time.hour:02}/00/"
        ...
        for line in lines:
            if "REQUEST {" in line:
                ...
                logs[id_log] = {
                    ...
                    "component": _component,  # Se puede asignar un valor fijo o dinámico si se conoce
